...
RETURNS TABLE(score double precision, song_id uuid, song_name varchar(100), artist_name varchar(100), album_name varchar(100), album_thumbnail varchar(100), album_dominant_color varchar(15))
LANGUAGE plpgsql
AS $$
    BEGIN
    RETURN QUERY SELECT s.score, s.id, s.name, a.name, alb.name, alb.thumbnail, alb.dominant_color from generate_series(0,5,0.05) indexScore
    CROSS JOIN LATERAL  (SELECT s.score, s.id, s.name, s.artist_id, s.album_id from Song s where s.score = indexScore and s.id != songId LIMIT 1) s
